...
# Clone upstream, apply patches, copy config.h, and build.
# Used for tools that still use the patch-based workflow (dmenu, dwmblocks).
build_suckless() {
    local name=$1
    local repo=$2
    local version=${3:-""}
...
# dwm: full source is in the repo (no clone, no patches needed)
build_dwm() {
